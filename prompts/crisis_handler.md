...
危机求助常发生在情绪最激动时（半夜连发、反复追问"她为什么不回我"）。诊断危机前先跑 `prompts/burnout_precheck.md`；命中双命中（高频 + 焦虑词）时，先陪伴、引导 `/simp quit`，不急着给危机话术。
...
> 前提：**明确拒绝是最高优先级硬约束**（见 SKILL.md 运行规则 4）。"我把你当朋友""我们只是朋友""我不喜欢你""我喜欢别人"都算**明确拒绝**——默认动作是优雅接受 + 递减脱离，**不得输出任何推进/追求型建议**。下面的分型只用来判断"多久以后是否值得重新评估"，**不用来决定"现在还能不能追"**（答案始终是：不能）。

**类型 A：明确型**（"我不喜欢你"、"我喜欢别人"、"我把你当朋友"、"我们只是朋友"）
→ 接受。这是终点，继续追只会同时伤害你和对方。

**类型 B：时机/带条件型**（"我现在没想谈感情"、"我现在还没准备好"、"你很好但..."、"我们现在不合适"）
→ 同样先接受、先脱离。有没有"以后"，由对方之后的主动决定，不由你现在的解读决定（见下方"评估与决策"）。
...
**默认结论：放下。** 被拒后的默认路径就是上面的"战略收缩/递减脱离"，然后把这页翻过去。"继续追"不是默认选项。

**唯一的例外 —— 仅当以下条件全部满足，才考虑"重新认识"（注意：不是继续追）：**
- 拒绝属于时机型（"现在不合适/还没准备好"），不是明确型（"不喜欢你/只是朋友/喜欢别人"）；
- **是对方主动**发出了新的、明确的互惠信号（主动找你并明确表达兴趣），而不是你解读出来的"她好像没那么排斥了"；
- 你能坦然接受"可能再被拒一次"，且不会因此纠缠。

**重新认识的方式**（若上述条件全部满足）：
- 以一个**全新、对等**的起点开始，频率和强度都明显低于上次，并随时间继续递减；
- 任何"她还没明确回应、我再推一把"的冲动，一律按明确拒绝处理——停下。
