...
在生成今日建议前，先跑统一的防耗尽预检。**触发条件、劝歇格式、坚持策略、无档案兜底全部以 `prompts/burnout_precheck.md` 为准**（范围＝全部 `/simp` 子指令，OR 逻辑）。命中任一高风险条件，**直接进入「劝歇」分支**，跳过 Step 2-4 的常规建议。

`/simp daily` 常见触发：7 天内 `/simp` 系列合计调用 ≥ 10 次、同一天 `/simp daily` ≥ 3 次、焦虑词密度高、评分连降。完整判定见 `burnout_precheck.md`。
...
**坚持策略以 `prompts/burnout_precheck.md` 为准**：单一触发可礼貌坚持一次、用户再坚持则给常规建议 + 警告；但**高频与焦虑词双命中时不让步**，只做陪伴并引导 `/simp quit`。
